...
import com.example.satellitefinder.databinding.ActivityAngleMeterBinding
...
class AngleMeterActivity : AppCompatActivity() {

    private val binding by lazy { ActivityAngleMeterBinding.inflate(layoutInflater) }
    private var torchEnable = false
    private var cameraEnable = false
    private var backgroundEnable = true
    private var lockEnable = false
    ...
    private fun setupClickListeners() = with(binding) {
        ...
        ifvLock.setOnClickListener {
            lockEnable = !lockEnable
            gonioView.setLock(lockEnable)
            showToast(if (lockEnable) "Locked" else "Unlocked")
            ifvLock.setImageResource(
                if (lockEnable) R.drawable.ic_lock_enable else R.drawable.ic_lock_disable
            )
        }
    }
    ...
    private fun showBannerAd() {
        if (canWeShowAds(RemoteConfig.angleMeterBanner)) {
            ...
            BannerAdUtils(activity = this, screenName = "level")
                .loadBanner(
                    ...
                    remoteConfig = RemoteConfig.angleMeterBanner,
